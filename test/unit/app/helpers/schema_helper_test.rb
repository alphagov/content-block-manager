  ...
  describe "#redirect_url_for_subschema" do
    ...
    context "when the subschema has a group" do
      ...
      it "should generate a url with the subschema's step" do
        assert_equal redirect_url_for_subschema(subschema, edition), workflow_path(edition, step: "#{Workflow::Step::SUBSCHEMA_PREFIX}#{subschema.id}")
      end
    end
    ...
    context "when the subschema has no group" do
      ...
      it "should generate a url with the subschema's group" do
        assert_equal redirect_url_for_subschema(subschema, edition), workflow_path(edition, step: "#{Workflow::Step::GROUP_PREFIX}#{subschema.group}")
      end
    end
  end
end
