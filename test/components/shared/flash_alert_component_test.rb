...
class Shared::FlashAlertComponentTest < ViewComponent::TestCase
  ...
  it "renders an error alert with the correct message" do
    render_inline(Shared::FlashAlertComponent.new(message: flash[:message]))
  ...
  it "allows HTML tags in the flash message when html_safe: true" do
    ...
    render_inline(Shared::FlashAlertComponent.new(message: flash[:message], html_safe: true))
