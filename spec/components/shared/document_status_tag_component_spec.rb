RSpec.describe Shared::DocumentStatusTagComponent, type: :component do
  expected_colours = {
    ...
    awaiting_review: "teal",
    awaiting_factcheck: "magenta",
    scheduled: "blue",
    published: "green",
    superseded: "orange",
    deleted: "red",
  ...
  let(:state) { :draft }
  let(:component) { described_class.new(state: state) }
  ...
  it "displays a translated version of the given state" do
    render_inline(component)
  ...
  describe "#colour" do
    Edition.new.available_states.each do |state|
      it "returns '#{expected_colours.fetch(state)}' for state '#{state}'" do
        expect(described_class.new(state: state).colour).to eq(expected_colours.fetch(state))
      end
    end
  ...
  describe "applies Design System colour styling successfully to govuk-tag" do
    Edition.new.available_states.each do |state|
      it "sets the tag--{colour} '#{expected_colours.fetch(state)}' for state '#{state}'" do
        render_inline(described_class.new(state: state))
