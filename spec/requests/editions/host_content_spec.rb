RSpec.describe "Host content", type: :request do
  ...
  describe "#form_handler" do
    ...
    it "redirects to the response_location_path" do
      form_handler_mock = instance_double(FormRedirectExtractor, response_location_path: "/preview/123")

      expect(FormRedirectExtractor).to receive(:new)
                                             .with(
                                               url: params[:url],
                                               form_body: body,
    ...
    it "returns a bad request if the service raises an unexpected response error" do
      expect(FormRedirectExtractor).to receive(:new)
                                       .and_raise(FormRedirectExtractor::UnexpectedResponseError)
