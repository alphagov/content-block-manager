RSpec.describe Block::Document::DocumentFilter do
  ...
  before do
    allow(Block::Document).to receive(:all).and_return(document_scope_spy)
    allow(document_scope_spy).to receive_messages(
      with_block_type: document_scope_spy,
      with_lead_organisation: document_scope_spy,
      last_updated_after: document_scope_spy,
      last_updated_before: document_scope_spy,
  ...
  describe "#call" do
    context "when no filters are given" do
      it "returns documents with default pagination and scopes applied" do
        allow(Block::Document).to receive(:with_keyword)

        filter

        expect(Block::Document).not_to have_received(:with_keyword)
        expect(document_scope_spy).not_to have_received(:where).with(id: anything)
        expect(document_scope_spy).to have_received(:with_block_type).with(nil)
        expect(document_scope_spy).to have_received(:with_lead_organisation).with(nil)
    ...
    context "when filtering by keyword" do
      ...
      before do
        allow(Block::Document).to receive(:with_keyword).and_return(keyword_relation)
      ...
      context "with standard text" do
        ...
        it "filters documents matching the target IDs" do
          ...
          expect(Block::Document).to have_received(:with_keyword).with("ministry of example")
      ...
      context "with an embed code containing an attribute reference" do
        ...
        it "strips the attribute reference before searching" do
          ...
          expect(Block::Document).to have_received(:with_keyword).with("{{embed:content_block_pension:basic-state-pension}}")
          expect(document_scope_spy).to have_received(:where).with(id: ids)
        end
      end
    ...
    context "when filtering by block_type" do
      ...
      it "delegates block_type filter to the query scope" do
        ...
        expect(document_scope_spy).to have_received(:with_block_type).with(%w[time_period])
      end
    end
    ...
    context "when filtering by last_updated dates" do
      it "applies starting date filter correctly" do
        ...
        expect(document_scope_spy).to have_received(:last_updated_after).with(Time.zone.local(2025, 2, 1))
      end

      it "applies ending date filter to end of day" do
        described_class.new(last_updated_to: { "1i" => "2026", "2i" => "4", "3i" => "3" }).call

        expect(document_scope_spy).to have_received(:last_updated_before).with(Time.zone.local(2026, 4, 3).end_of_day)
      end
    end
