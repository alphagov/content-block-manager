module Edition::Diffable
  extend ActiveSupport::Concern

  def generate_diff
    diff = {}
    unless document.is_new_block?
      diff["title"] = DiffItem.new(previous_value: previous_edition.title, new_value: title) if previous_edition.title != title
      diff["details"] = details_diff if details_diff.any?
      diff["lead_organisation"] = DiffItem.new(previous_value: previous_org.name, new_value: lead_organisation.name) if lead_organisation != previous_org
      diff["instructions_to_publishers"] = DiffItem.new(previous_value: previous_edition.instructions_to_publishers, new_value: instructions_to_publishers) if previous_edition.instructions_to_publishers != instructions_to_publishers
    end
    diff
  end
  ...
  def generate_details_diff(previous_details = previous_edition.details, current_details = details)
    ...
    keys.each do |key|
      ...
      if previous_value.is_a?(String) || new_value.is_a?(String)
        ...
        diff[key] = DiffItem.new(previous_value:, new_value:)
