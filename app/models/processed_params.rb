...
private
  ...
  def process_date_field(field, collection)
    ...
    month = pad_date_part get_and_remove_date_time_part_from_collection(field, "month", collection).presence
    day = pad_date_part get_and_remove_date_time_part_from_collection(field, "day", collection).presence

    "#{year}-#{month}-#{day}"
  end

  def pad_date_part(part)
    return part if part.nil?
    # Return string without padding if it's not a number
    return part if /\p{Alpha}/.match(part)
