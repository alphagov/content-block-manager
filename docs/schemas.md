...
A one-to-one embedded relationship is defined as a regular JSON object. (see the [Time period schema](https://github.com/alphagov/content-block-manager/blob/main/app/models/schema/definitions/time_period.json#L8))
for an example).

Using the time period schema as an example, the `date_range` payload looks like this:
...
Using the pension schema as an example, the `rates` payload looks like this:
