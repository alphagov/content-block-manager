# 9. Use events to record edition history
...
We will introduce a new `Event` model to explicitly record significant actions in an Edition's lifecycle. This 
decouples the **historical record** (what happened and when) from the **current state** (what the edition looks like now).

### Schema

An `Event` will capture the specific action (`name`) and a snapshot of relevant context (`metadata`) at the exact 
...
erDiagram
    ...
    editions {}
    versions {}
    events ||--}o editions : "belongs to"
    events |o--|| versions : "(optionally) belongs to"
```
...
field-level data changes (diffs), while the `Event` handles the semantic timeline.
...
When a fact-check is performed, we create an `Event`:
...
We will update the application to generate `Event` records for the following actions:
...
We will also modify the `has_audit_trail` concern to generate an `Event` alongside the existing `Version` when an 
edition is saved.

### Migration & Backfill

*   Existing `Edition`s must be backfilled. We will write a migration script to generate `Event` records based on
...
*   The `TimelineItemComponent` will be updated to read from the `Event` stream rather than `Version` records.
