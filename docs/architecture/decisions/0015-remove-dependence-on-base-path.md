...
[RFC-192: Restrict base_paths to a smaller character set][RFC-192] proposes restricting GOV.UK base paths to `a-z 0-9 . / -` (no underscores, no uppercase). Content blocks are affected because their `base_path` includes the `document_type` (e.g. `content_block_time_period`) which contains underscores. The `document_type` itself would not need to change, only the base path construction.

Two code locations construct the path:
...
The `base_path` is important in two scenarios. In both cases the "tools" gem's `ContentBlockTools::ContentBlock.from_embed_code(embed_code)` is used to retrieve a block from Content Store, as follows:
...
flowchart TD
    ...
    D["Revert PR 172 in CBM to publish blocks to Content Store"] --> E
    E["Deploy CBM + republish all blocks"] --> F
