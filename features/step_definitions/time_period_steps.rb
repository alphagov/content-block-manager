module TimePeriodHelpers
  ...
  def details_with_feb_29th_non_leap_year
    OpenStruct.new({
      "date_range" => {
        "start" => {
          "date" => "2025-02-28",
...
When("I supply the time periods with invalid dates") do
  @details = time_period.invalid_details
  time_period.fill_time_period_fields(details: @details, page: page)
end

When("I supply the time periods with an end date of 29th February in a non-leap year") do
  @details = time_period.details_with_feb_29th_non_leap_year
  time_period.fill_time_period_fields(
    details: @details,
    page: page,
  )
end
